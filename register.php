	...
	switch (strtolower($_GET['action'])) {
		...
		if (isset($_POST['username']) && isset($_POST['password'])) {
			if (createAccount($_POST['username'], $_POST['password'])) {
				...
				$_SESSION['error'] = "credentials not supplied.";
				unset($_GET['action']);
			}
		break;
	}
...
if (loggedIn()) {
	...
	registered and currently logged in as:'. $_SESSION['username'].'.
	<h4><a href="login.php?action=logout">logout</a>?</h4>
	<h4><a href="index.php">site index</a>?</h4>';
				

}
// If the action is not set,
// we want to display the registration form
elseif (!isset($_GET['action'])) {
    ...
    action="' . $_SERVER['PHP_SELF'] . '?action=register">
